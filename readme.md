...
`BookBuilder.SetProperties` sets the workbook's document properties — the standard fields Excel surfaces in the File > Info pane, plus any user-defined entries shown on the "Custom" tab of the Advanced Properties dialog. Every field is optional; only values that are set get written, so an unset property leaves that part of the workbook untouched.

<!-- snippet: DocumentPropertiesUsage -->
<a id='snippet-DocumentPropertiesUsage'></a>
```cs
var builder = new BookBuilder();
builder.AddSheet(SampleData.Employees());
...
The header row is marked as a repeating header (`w:tblHeader`), so a table long enough to break across a page keeps its heading on each later page.
...
```
<sup><a href='/src/Excelsior.Tests/Word/WordTableBuilderTests.cs#L655-L665' title='Snippet source file'>snippet source</a> | <a href='#snippet-WordTableBodyStyle' title='Start of snippet'>anchor</a></sup>
<!-- endSnippet -->
...
```
<sup><a href='/src/Excelsior.Tests/Word/WordTableBuilderTests.cs#L756-L762' title='Snippet source file'>snippet source</a> | <a href='#snippet-WordTableParagraphStyles' title='Start of snippet'>anchor</a></sup>
<!-- endSnippet -->
...
The table itself references Word's built-in `TableGrid` — single-line borders — unless told otherwise. `TableStyle` points it at a style defined in the host document instead, which is how a table takes its borders, banding, and cell margins from a branded template:

<!-- snippet: WordTableStyle -->
